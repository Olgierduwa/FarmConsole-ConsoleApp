...
namespace FarmConsole.Body.Views.GameViews
{
    class SideMenuView : ComponentService
    {
        ...
        public static CM DisplayCenterMenu(string content, string title)
        {
            ...
            title = title == "" ? LS.Navigation("warning") : LS.Navigation(title);
            text = new string[] { " ", title, " " };
            if (content.Split('\n').Length > 1) text2 = content.Split('\n');
            else text2 = GetTextBoxView(content, Width);
            ...
            GroupStart(3);
            TextBoxLines(text, Width, Background: color);
            GroupEnd();
            PrintList();
        ...
        public static CM DisplayRightMenu(List<ProductModel> component_list, string title, int Selected, bool extended = false, string filter = "")
        {
            ...
            if (component_list.Count > 0)
            {
                ...
                {
                    var amount = component_list[i].Amount;// / (component_list[i].Slots < 0 ? (component_list[i].Slots * -1) : 1);
                    if (i * 3 + 3 < Height * 3)
                        TextBox(amount + "x " + LS.Object(component_list[i].ObjectName), Width, Margin: 0);
                    else TextBox(amount + "x " + LS.Object(component_list[i].ObjectName), Width, false, Margin: 0);
                }
            }
